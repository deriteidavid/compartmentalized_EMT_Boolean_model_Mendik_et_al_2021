...
from boolean2_modified import tokenizer, util, state
from boolean2_modified.ply import yacc
...
def p_stmt_assign(p):
    ...
    import noise_config
    importlib.reload(noise_config)
    #print(noise_config.epsilon)
    p4_old=p[4]
    p[4] = 2*int(np.random.binomial(1, p=sigmoid(epsilon=noise_config.epsilon, x=p[4])))-1
    #print (p[1],sigmoid(epsilon=epsilon, x=p[4]), p[4])
...
class Parser(object):
    ...
    def __init__(self, mode, text ):
        ...
        self.parser.RULE_NOT = lambda a, p: (-1) * a #-1 1 case
        #self.parser.RULE_NOT = lambda a, p: 1-a #boolean case
