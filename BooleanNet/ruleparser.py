...
PLDE, SYNC, ASYNC, RANK, TIME, ROA, WROA, WASYNC = 'plde sync async rank time roa wroa wasync'.split()

# valid modes of operation
VALID_MODES = [PLDE, SYNC, ASYNC, RANK, TIME, ROA, WROA, WASYNC]

# weighed modes of operation
WEIGHTED_MODE = [WROA, WASYNC]

# the labels will be set to 1 for these
NOLABEL_MODE = [PLDE, SYNC, ASYNC, ROA]
...
def p_expression_id(p):
    ...
    # this is the only distinction between synchronous and asynchronous updating
    if p.parser.sync:
...
def p_expression_binop(p):
    ...
    if p[2] == 'and':
        ...
        p[0] = p.parser.RULE_XOR(p[1], p[3], p)
    elif p[2] == '*':
        p[0] = p.parser.RULE_TIMES(p[1], p[3], p)
    else:
...
class Parser(object):
    ...
    def __init__(self, mode, text):
        ...
        if mode in WEIGHTED_MODE:
            self.parser.weighted = True
        ...
        def rule_xor(a, b, p):
            if float(a) == 0.0 and float(b) > 0.0:
                return b
            elif float(a) > 0.0 and float(b) == 0.0:
                return a
            else:
                return 0.0
        ...
        #
        self.parser.RULE_AND = lambda a, b, p: min(float(a), float(b))
        self.parser.RULE_OR = lambda a, b, p: max(float(a), float(b))
        self.parser.RULE_XOR = rule_xor
        self.parser.RULE_TIMES = lambda a, b, p: a * b
        ...
        # build another parsable text, as lines stored for rank keys
